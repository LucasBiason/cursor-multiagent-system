...
def load_env_passwords():
    ...
    1. config/.env.passwords (preferred - subrepositório privado)
    ...
    """
    # Get project root (assuming script is in core/scripts/infrastructure/)
    project_root = Path(__file__).parent.parent.parent.parent
    
    # Try config/.env.passwords first (subrepositório privado)
    env_paths = [
        project_root / "config" / ".env.passwords",
...
def get_deployment_credentials(project: str) -> dict:
    """
    Get deployment server credentials for a specific project.
    
    Args:
        project: Project name (e.g., expenseiq, kpi, portfolio)
    
    Returns:
        dict: Credentials with keys: server, ip, ssh_user, ssh_host, password, github_repo, branch
    """
    project_upper = project.upper()
    
    return {
        "server": os.getenv(f"DEPLOYMENT_{project_upper}_SERVER"),
        "ip": os.getenv(f"DEPLOYMENT_{project_upper}_IP"),
        "ssh_user": os.getenv(f"DEPLOYMENT_{project_upper}_SSH_USER", "root"),
        "ssh_host": os.getenv(f"DEPLOYMENT_{project_upper}_SSH_HOST"),
        "password": os.getenv(f"DEPLOYMENT_{project_upper}_PASSWORD"),
        "github_repo": os.getenv(f"DEPLOYMENT_{project_upper}_GITHUB_REPO"),
        "branch": os.getenv(f"DEPLOYMENT_{project_upper}_BRANCH", "main"),
    }
...
if __name__ == "__main__":
    ...
    # Get credentials
    deployment_creds = get_deployment_credentials("expenseiq")
    render_creds = get_render_credentials()
    notion_creds = get_notion_credentials()
    
    print("\n📋 Loaded Credentials:")
    print(f"Deployment IP: {deployment_creds.get('ip', 'NOT SET')}")
