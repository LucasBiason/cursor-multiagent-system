...
if [ -f "core/scripts/infrastructure/validate.sh" ]; then
    bash core/scripts/infrastructure/validate.sh
fi
...
echo "2. Configure Notion IDs in MCP project: my-local-place/services/external/notion-automation-suite/config/.env"
echo "3. Set NOTION_API_KEY environment variable"
echo "4. Run make validate to verify setup"
echo ""
